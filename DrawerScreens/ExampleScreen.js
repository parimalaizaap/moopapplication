  ...
  return (
    <View>
      ...
      <TouchableOpacity
        ...
        style={styles.header}>
        <View > 
          <Checkbox.Item  color='black' style={{ width: 50, height: 40 }}  />  
         </View>
...
const App  = ({navigation}) => {
  ...
  const [CurrentOption, setCurrentOption] = useState("Bulk Edit");
  const [name, setName] = useState("gggggggg")
  ...
  React.useLayoutEffect(() => {
   ...
   handleClick = () => {
    ...
    if(strCompare(CurrentOption,'Bulk Edit'))
    {
    setCurrentOption('Delete') 
   }else{
    setCurrentOption('Bulk Edit');
   }
   console.log(CurrentOption);
    ...
    navigation.setOptions({
      headerRight: () => (
        <Text style={styles.headerText} onPress={ setName("Dhuvaraga")}>{name}</Text>
